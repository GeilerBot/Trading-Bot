...
url = 'https://discord.com/api/webhooks/853609816139104296/9mGR2x1-hv2xHSuWrZTKW_MKARrU5g--MaGAV4NTNHWRo6o3UsM64ueLAsL-nJoI5C6-'
...
def trailingStop(price):
    ...
    if bitcoin_position <= stop_loss and stop_loss != 0:
        ...
        embed1 = DiscordEmbed(title='Sell DogeCoin', description=f'Sold DogeCoin on trailing-stop', color=0xffb029)
        embed1.set_thumbnail(url='https://images-ext-2.discordapp.net/external/e39qYTNcnVKw1Iat-rBMbv-X0VaOZSQM96SJNvo7lhQ/https/upload.wikimedia.org/wikipedia/en/d/d0/Dogecoin_Logo.png')
        ...
        embed1.add_embed_field(name='DogeCoin Price:', value=f'${str(round(price))}', inline=True)
        embed1.add_embed_field(name='Portfolio:', value=f'{str(round(real_time_portfolio_value))}/{str(amount)}', inline=True)
        embed1.set_footer(text=f'Sold DogeCoin at {date}')
        ...
        def on_message(ws, message):
            ...
            if candle_closed:
                ...
                if amt > 0 and amt < money_end:
                    ...
                    embed2 = DiscordEmbed(title='Bought DogeCoin', description=f'Bought DogeCoin on "{pattern}" strategy', color=0xffb029)
                    embed2.set_thumbnail(url='https://images-ext-2.discordapp.net/external/e39qYTNcnVKw1Iat-rBMbv-X0VaOZSQM96SJNvo7lhQ/https/upload.wikimedia.org/wikipedia/en/d/d0/Dogecoin_Logo.png')
                    ...
                    embed2.add_embed_field(name='DogeCoin Price:', value=f'${str(round(last_price))}', inline=True)
                    embed2.add_embed_field(name='Portfolio:', value=f'{str(round(real_time_portfolio_value))}/{str(amount)}', inline=True)
                    embed2.set_footer(text=f'Bought DogeCoin at {date}')
