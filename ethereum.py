...
url = 'https://discord.com/api/webhooks/853609382507315210/8Gm-xJQieeE5sf7j_yAqu-XNZZJQ6IuPsyl33LNdvP_qFQqqnQdYxu2ZR6hvQQJSrb_f'
...
def trailingStop(price):
    ...
    if bitcoin_position <= stop_loss and stop_loss != 0:
        ...
        embed1 = DiscordEmbed(title='Sell Ethereum', description=f'Sold Ethereum on trailing-stop', color=0xffb029)
        embed1.set_thumbnail(url='https://images-ext-1.discordapp.net/external/tyhQNSkCFcft-LTqjcX7bo9UEmI4DceCxvK8c1p4BHw/https/upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Ethereum-icon-purple.svg/1200px-Ethereum-icon-purple.svg.png?width=1042&height=1042')
        ...
        embed1.add_embed_field(name='Ethereum Price:', value=f'${str(round(price))}', inline=True)
        embed1.add_embed_field(name='Portfolio:', value=f'{str(round(real_time_portfolio_value))}/{str(amount)}', inline=True)
        embed1.set_footer(text=f'Sold Ethereum at {date}')
        ...
        def on_message(ws, message):
            ...
            if candle_closed:
                ...
                if amt > 0 and amt < money_end:
                    ...
                    embed2 = DiscordEmbed(title='Bought Ethereum', description=f'Bought Ethereum on "{pattern}" strategy', color=0xffb029)
                    embed2.set_thumbnail(url='https://images-ext-1.discordapp.net/external/tyhQNSkCFcft-LTqjcX7bo9UEmI4DceCxvK8c1p4BHw/https/upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Ethereum-icon-purple.svg/1200px-Ethereum-icon-purple.svg.png?width=1042&height=1042')
                    ...
                    embed2.add_embed_field(name='Ethereum Price:', value=f'${str(round(last_price))}', inline=True)
                    embed2.add_embed_field(name='Portfolio:', value=f'{str(round(real_time_portfolio_value))}/{str(amount)}', inline=True)
                    embed2.set_footer(text=f'Bought Ethereum at {date}')
