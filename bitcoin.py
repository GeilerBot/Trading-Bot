...
url = 'https://discord.com/api/webhooks/853609278199955456/cArrDTfrdaoBYhAugpFPv6Z_f_bbQ-lAExG6PlWDjGHg0OcwFXzsqwOuZy5XgxtvS2Ii'
...
def trailingStop(price):
    ...
    if bitcoin_position <= stop_loss and stop_loss != 0:
        ...
        embed1 = DiscordEmbed(title='Sell Bitcoin', description=f'Sold Bitcoin on trailing-stop', color=0xffb029)
        embed1.set_thumbnail(url='https://media.discordapp.net/attachments/792024444742533121/856218171950628894/bitcoin_.png')
        ...
        embed1.add_embed_field(name='Bitcoin Price:', value=f'${str(round(price))}', inline=True)
        embed1.add_embed_field(name='Portfolio:', value=f'{str(round(real_time_portfolio_value))}/{str(amount)}', inline=True)
        embed1.set_footer(text=f'Sold Bitcoin at {date}')
...
class Bot(Thread):
    def run(self):
        def on_message(ws, message):
            global portfolio, investment, closes, highs, lows, opens, money_end, real_time_portfolio_value, timer, stop_loss, crypto, pattern, securitynumber
            ...
            if candle_closed:
                ...
                if amt > 0 and amt < money_end:
                    ...
                    embed2 = DiscordEmbed(title='Bought Bitcoin', description=f'Bought Bitcoin on "{pattern}" strategy', color=0xffb029)
                    embed2.set_thumbnail(url='https://media.discordapp.net/attachments/792024444742533121/856218171950628894/bitcoin_.png')
                    ...
                    embed2.add_embed_field(name='Bitcoin Price:', value=f'${str(round(last_price))}', inline=True)
                    embed2.add_embed_field(name='Portfolio:', value=f'{str(round(real_time_portfolio_value))}/{str(amount)}', inline=True)
                    embed2.set_footer(text=f'Bought Bitcoin at {date}')
